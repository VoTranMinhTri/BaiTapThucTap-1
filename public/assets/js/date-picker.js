...
class DatePicker extends HTMLElement {
    ...
    connectedCallback() {
        ...
        var calendar = this.shadow.querySelector('.calendar-dropdown');
        btndate.onclick = function() {
            if(calendar.className.includes('visible') == false){
                calendar.className +=' visible';
            }
            else{
                calendar.className = calendar.className.replace(' visible','');
            }
        }
    }
    ...
    updateToggleText() {
        ...
        inputdate.value = this.toggleButton.textContent;
    }
...
<button type="button" class="date-toggle" title="Ngày sử dụng">${date}</button>
